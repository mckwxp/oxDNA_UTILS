...
void RepulsionPlane<number>::get_settings (input_file &inp) {
	...
	getInputNumber(&inp, "xmin", &_xmin, 1);
	getInputNumber(&inp, "ymin", &_ymin, 1);
	getInputNumber(&inp, "zmin", &_zmin, 1);
	getInputNumber(&inp, "xmax", &_xmax, 1);
	getInputNumber(&inp, "ymax", &_ymax, 1);
	getInputNumber(&inp, "zmax", &_zmax, 1);
...
LR_vector<number> RepulsionPlane<number>::value(llint step, LR_vector<number> &pos) {
	...
	else if((pos.x <= this->_xmin || pos.x >= this->_xmax) || (pos.y <= this->_ymin || pos.y >= this->_ymax) || (pos.z <= this->_zmin || pos.z >= this->_zmax) ) return LR_vector<number>(0., 0., 0.);
...
number RepulsionPlane<number>::potential(llint step, LR_vector<number> &pos) {
	...
	else if((pos.x <= this->_xmin || pos.x >= this->_xmax) || (pos.y <= this->_ymin || pos.y >= this->_ymax) || (pos.z <= this->_zmin || pos.z >= this->_zmax) ) return 0.;
