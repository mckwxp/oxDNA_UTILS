		...
		for(int i = 0; i < this->_N; i++) {
			...
			for(int j = 0; j < p->N_ext_forces; j++) {
				...
				if(typeid(*(p->ext_forces[j])) == typeid(const_force) ) {
					...
					COMForce<number> *p_force = (COMForce<number> *) p->ext_forces[j];
					...
					force->comforce.n_com = p_force->_n_com;
					force->comforce.n_ref = p_force->_n_ref;
					for (int i = 0; i < p_force->_n_com; i++){
						force->comforce.com_indexes[i] = p_force->_com_indexes[i];
					}
					for (int i = 0; i < p_force->_n_ref; i++){
