		...
		switch (extF.type) {
			...
			case CUDA_COM_FORCE: {
				...
				for(int index = 0; index < extF.comforce.n_com; index++){
					int index2 = extF.comforce.com_indexes[index];					
					com += poss[index2];
				}
				...
				number4 ref = make_number4<number,number4>(0.,0.,0.,0.);
				for(int index = 0; index < extF.comforce.n_ref; index++){
					int index2 = extF.comforce.ref_indexes[index];
					ref += poss[index2];
				}
				...
				number4 dr = ref - com;
				number dr_abs = _module<number, number4>(dr);
				number4 force = dr * ((dr_abs - (extF.comforce.r0 + extF.comforce.rate * step)) * extF.comforce.stiff / dr_abs) / extF.comforce.n_com;
