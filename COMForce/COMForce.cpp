...
void COMForce<number>::init(BaseParticle<number> **particles, int N, BaseBox<number> * box_ptr) {
	...
	//_com_indexes = &com_indexes[0]; // cast vector to int[] so that it can be passed to device code
	...
	_n_ref = ref_indexes.size();
	for(int i = 0; i < _n_ref; i++){
		_ref_indexes[i] = ref_indexes[i];
	}
	//_ref_indexes = &ref_indexes[0]; // cast vector to int[] so that it can be passed to device code
	for(vector<int>::iterator it = ref_indexes.begin(); it != ref_indexes.end(); it++) {
		_check_index(*it, N);
		_ref_list.insert(particles[*it]);
	}
	
	OX_LOG(Logger::LOG_INFO, "Adding a COM force of stiffness = %lf, r0 = %lf, rate = %f", this->_stiff, _r0, _rate);
}
...
LR_vector<number> COMForce<number>::value(llint step, LR_vector<number> &pos) {
	...
	number force = (d_com - (_r0 + _rate * step)) * this->_stiff / _com_list.size();
...
number COMForce<number>::potential(llint step, LR_vector<number> &pos) {
	_compute_coms(step);
	return 0.5 * this->_stiff * SQR((_ref_com - _com).module() - (_r0 + _rate * step)) / _com_list.size();
}
