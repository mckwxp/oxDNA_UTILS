...
#        exit(1)
    ...
    def get_1d_vecs(self, discard_unbonded = True):
        ...
        The returned vectors are "invariant" co-ordinates x', y', z'.
        x': along double helix; 
        yy: average backbone-base vector for the 0th strand; 
        z': x' cross yy; 
        y': z' cross x'

        Args:
            discard_unbonded: boolean. If true, discard broken base pairs in the calculation. NB: it is not currently implemented in this function.
        """
    ...
    def get_3d_vecs(self, vhelix_extent = False, discard_unbonded = True):
        ...
        NB: the author of this documentation believes that it is actually used. Perhaps the author of the code did not update the comments after vhelix_extent was implemented.
    ...
    def get_plane_vecs(self, discard_unbonded = True):
        """
        This function was not documented when it was written.
        The author of this documentation believes that it sets self.vec_long and self.vec_lat for an origami_utils.Origami object.
    ...
    def get_h_bond_list(self, infile):
        """
        Fill in self.interaction_list for the current system self._sys; 
        self.interaction_list[nucid1] is a list of nucleotides that nucid1 has hydrogen bonds with

        Args:
            infile: path to input file for DNAnalysis
        """
    ...
    def get_cad2cudadna(self, infile, visibility = False):
        """
        This function was not documented when it was written.
        The author of this documentation believes that it is used for parsing the virt2nuc file when initialising an origami object.
    ...
    def get_holliday_junctions(self, single=False, single_only=False):
        """
        Returns a list of hjs in format (vh, vb, vh_neighbour, vb_next). Assumes no insertions/deletions when using cad2cudadna.
    ...
    def get_vhelix_neighbours(self, type):
        """
        This function was not documented when it was written.
        The author of this documentation believes that it returns a neighbour list of virtual helices.

        Args:
            type: "sq" or "he", corresponding to square or hexagonal lattice in cadnano
        """
    ...
    def get_weave(self, verbose=False):
        """
        Returns the weave pattern as a list of lists. Each pair of virtual 
        ...
        Args:
            verbose: boolean. If true, prints skipped virtual helix pairs.
        """
    ...
    def get_vh_midpoints(self, vhi):
        """
        Returns a list of base pair midpoints for a virtual helix. Effectively 
    ...
    def get_arm_vec(self, vhc,vbc,vbn):
        """
        This function was not documented when it was written.
        The author of this documentation believes that it returns a vector along an arm of a Holliday junction, 
        or more generally, a vector pointing along a virtual helix.
    ...
    def intra_helix_autocorr(self):
        """
        Returns the autocorrelation (i.e. the dot product) between the intra-helix 
    ...
    def corrugation(self, hj, span=16):
        """
        Returns the corrugation a distance span around the junction hj. The 
    ...
    def corrug_angle(self, xpos, bbms1, bbms2, id1, id2, av_axis, ref_intra_in_plane):
        """
        Computes the corrugation angle as the dot product of the intra-strand
    ...
    def old_get_corrug_weave(self, vh1, vh2, vb, n, norm2, corrug_signed=False):
        """
        Returns distance between adjacent helices and decomposes into weave and corrugation 
        for a pair of base pairs located at (vh1, vb) and (vh2, vb). Assumes no skip/loop.
        """
        ss = self._sys
    ...
    def get_n_nicks(self):
        """
        Returns the number of nicks (staple or scaffold) in an origami. 
    ...
    def get_nicks(self):
        """
        Returns a list [scaffold_nicks, staple_nicks], each of which is a list of nicked neighbours in the origami.
        Here a nick must be between adjacent strands (e.g. if a nucleotide is missing that isn't counted as a nick at all).
    ...
    def parse_vhelix_extent_file(self, vhelix_extentf):
        """
        Parses the vhelix_extent file and returns a list [vh_begin, vh_end], where vh_begin is a list of length
    ...
    def parse_vh_edge(self, f_edge_vh):
        ...
        Args:
            f_edge_vh: path to vh_edge file
        """
        try:
            f = open(f_edge_vh, "r")
        except IOError:
